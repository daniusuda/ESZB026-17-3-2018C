...
void setup(){
  ...
  TWBR=10000L;                    // a frequencia do i2c: 100000L = 100kHz
...
void enviaDadosPelaSerial(){              // opcional, para debug
  ...
  Serial.print("; valor de tens: "); Serial.print(potencial);
  ...
  Serial.print(buffer[5]); Serial.print(" "); // byte mais significativo do alarme
  Serial.println(buffer[6]);//byte mesnos significativo em volts
}

void loop(){                       // Le o valor no potenciometro a cada 1 segundo
  valorPot = analogRead(PinoPotenciometro);      // le valor no pino A0 usando conversor ADC de 10-bit
  potencial = valorPot * (5.0f/1023.0f); // converte o valor lido em potencial
  buffer[0] = valorPot & 0xFF;     // byte menos significativo
  buffer[1] = valorPot >> 8;       // byte mais significativo
  buffer[5] = (int)potencial;     // byte menos significativo
  buffer[6] = (int)((potencial-buffer[5])*100.0f);       // byte mais significativo
...
void funcaoDadoRecebido(int x){    // funcao chamada ao receber um dado
  ...
  if( dadoRecebido == 0x02 && x == 3 ){  // se for alteracao do limite do alarme
    ...
    byte dado3 = Wire.read();
    byte dado4 = Wire.read();
    int valorRecebido =  dado1 | ( dado2 << 8 )| dado3 | dado4;
